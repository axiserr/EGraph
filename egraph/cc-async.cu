...
int main(int argc, char** argv)
{
	...
	*/
	...
	// }
				...
				// sssp_async<<<partitioner.partitionNodeSize[i]/512 + 1,512,0,stream0>>>(partitioner.partitionNodeSize[i],//host   partitioner.partitionNodeSize[i]/512 + 1 , 512
				// 									partitioner.fromNode[i],//host
				// 									partitioner.fromEdge[i],//host
				// 									subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
				// 									subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
				...
				// 									(itr%2==1) ? graph.d_label1 : graph.d_label2,//cudamemcpy
				// 									(itr%2==1) ? graph.d_label2 : graph.d_label1);		//cudamemcpy		
				// 									//readtime=timer1.Finish();
				// 									//cout<<"stream1 processing finished in" <<readtime<<"(s).\n";
				...
				// //cudaDeviceSynchronize();			
				// // timer2.Start();
				// // readtime1=timer2.Start();
				// //start1 = clock();
				// //cout<<"stream2 processing start in" <<start1<<"(s).\n";
				// //cout<<"stream2 processing start in" <<readtime1<<"(s).\n";
				// sssp_async<<<partitioner.partitionNodeSize[i]/512 + 1, 512,0,stream1>>>(partitioner.partitionNodeSize[i],//host
				// 										partitioner.fromNode[i],//host
				// 										partitioner.fromEdge[i],//host
				// 										subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
				// 										subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
				...
				// 										graph.d_value1, //cudamemcpy
				// 										d_finished1,//wait
				// 										(itr%2==0) ? graph.d_label11 : graph.d_label22,//cudamemcpy
				// 										(itr%2==0) ? graph.d_label22 : graph.d_label11);//cudamemcpy
				// 										//readtime=timer2.Finish();
				// 										//cout<<"stream2 processing finished in" <<readtime<<"(s).\n";

							//printf("size=%u?????????????????????\n",partitioner.partitionNodeSize[0]);
			...
			// 		partitioner.fromNode[i],//host
			// 		partitioner.fromEdge[i],//host
			// 		subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值，存在GPU
			// 		subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值，存在GPU
			// 		subgraph.d_activeEdgeList,  //cudamemcpy   存在CPU，CPU-GPU的传输
			// 		graph.d_outDegree,//cudamemcpy
			// 		graph.d_value, //cudamemcpy
			// 		d_finished,//wait
			// 		(itr%2==1) ? graph.d_label1 : graph.d_label2,//cudamemcpy
			...
			// 		//printf("^^^^^^^^^^^^^^^^^^^^\n");
			// 		cout<<"finished/finished1="<<finished<<""<<finished1<<endl;
			// }while(!(finished)||!(finished1)); // finished=ture，finished=false才会循环 ,  ||finished1	